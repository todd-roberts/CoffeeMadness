...
import {
    ...
    SCORE_SIZE, WEST_X, SOUTH_X, WEST_TEXT_COLOR, SOUTH_TEXT_COLOR, WINNER_TEXT_COLOR
} from './text';
...
class Main extends Phaser.Scene {
    ...
    init() {
        ...
        this.game.socket.on('gameOver', nextGameAt => {
            const winner = this.scores.west >= this.scores.south ? 'WEST' : 'SOUTH';
    ...
    createOtherPlayers() {
        ...
        this.game.socket.on('playerJoined', playerName => {
            ...
            this.otherPlayers[playerName] =
                this.physics.add.staticSprite(-100, 0, playerName);
        });
        ...
        this.game.socket.on('update', update => {
            if (!this.otherPlayers[update.name]) {
                this.otherPlayers[update.name] =
                    this.physics.add.staticSprite(-100, 0, update.name);
            }

            if (update.tintTopLeft) console.log(update);

            Object.entries(update).forEach(([k, v]) => {
                this.otherPlayers[update.name][k] = v;
            });

            if (update.tintTopLeft == 42495) {
                this.otherPlayers[update.name].setTint(HYPER);
            }
        });
    }
    ...
    addScoresText() {
        ...
        this.southText = this.add.bitmapText(
            SOUTH_X,
            y,
            FONT,
            `S:${south}`,
        ...
        this.southText.setTint(SOUTH_TEXT_COLOR);

        this.southText.setScrollFactor(0);
    }
    ...
    addAbsorbUpdates() {
        this.game.socket.on('collect', ({ player, sprite, id, west, south, poison, hyper }) => {
    ...
    updateScores() {
        ...
        this.westText.y = y;
        this.southText.setText(`S:${this.scores.south}`);
        this.southText.y = y;
    }
